<?php
/** * FORCEKES - menu.php (Fase 10: DB Driven Sorting) */
...
// Sorteer op de 'priority' kolom die we in de admin hebben ingesteld
if (!empty($navAlbums)) {
...
<aside id="explorer-panel" class="explorer-panel fixed top-0 right-0 bottom-0 w-full max-w-sm border-l border-white/5 z-[120] p-16 overflow-y-auto">
    <header class="mb-20 flex justify-between items-center">
        <p class="text-[9px] font-black uppercase tracking-[0.4em] text-blue-600 italic">Archief Verkenner</p>
        <button onclick="toggleExplorer()" class="text-zinc-600 hover:text-white transition"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
    </header>
    <nav class="space-y-10">
        <?php foreach ($navAlbums as $album): ?>
            <a href="gallery.php?page=<?= rawurlencode($album['category_name']) ?>" class="group block border-b border-white/5 pb-6">
                <span class="text-[8px] font-black text-zinc-600 uppercase tracking-widest block mb-2"><?= (int)$album['photo_count'] ?> Media</span>
                <span class="serif-italic text-3xl group-hover:text-blue-500 transition-all block"><?= ucfirst($album['category_name']) ?></span>
            </a>
        <?php endforeach; ?>
    </nav>
</aside>
