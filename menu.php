...
$feesten = is_array($feestenRaw) ? $feestenRaw : [];
...
.nav-link { font-size: 13px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.2em; color: #fff; transition: 0.4s; cursor: pointer; }
.menu-panel { transform: translateX(100%); transition: transform 0.5s cubic-bezier(0.16, 1, 0.3, 1); background: #020202; z-index: 1000; width: 400px; max-width: 90vw; border-left: 1px solid rgba(255,255,255,0.1); }
...
<a href="index.php" class="text-lg font-black tracking-tighter text-white">Force<span class="text-blue-600">kes</span> Portaal</a>
...
<?php if($isLoggedIn): ?><a href="profiel.php" class="nav-link text-zinc-500">MIJN COCKPIT</a><?php endif; ?>
</div>
</div>
</nav>
...
<a href="login.php" class="menu-item text-zinc-600"><?= $isLoggedIn ? 'LOGOUT' : 'TOEGANG' ?></a>
</nav>
</div>
...
document.querySelectorAll('.menu-level').forEach(el => { el.classList.remove('active', 'push-left', 'push-right'); el.classList.add('push-right'); });
const target = document.getElementById('level-' + level);
target.classList.remove('push-right'); target.classList.add('active');
