...
// 2. BATCH RENAME LOGICA (Tegen 504 Time-outs)
if (isset($_GET['action']) && $_GET['action'] === 'batch_rename') {
    $old = $_GET['old'];
    $new = strtolower(trim($_GET['new']));
    $limit = 12; // Kleine hapjes van 12 foto's
    ...
    if (is_array($items) && count($items) > 0) {
        ...
        $statusMessage = "Verhuizing voltooid: '$old' is nu '$new'.";
    }
}

// 3. POST ACTIES (Delete & Rename Start)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'delete_album') {
        ...
        $statusMessage = "Album '$slug' gewist uit database.";
        $statusType = "error";
    }
}
...
<html lang="nl">
<head>
    <meta charset="UTF-8"><title>Forcekes Admin | Dashboard</title>
    ...
    <main class="max-w-7xl mx-auto px-6 py-32">
        ...
        <?php if (isset($_GET['status']) && $_GET['status'] === 'processing'): ?>
            <div class="mb-12 p-10 glass rounded-[3rem] border border-blue-600/30 text-center">
                ...
                <h2 class="text-xl font-black uppercase italic tracking-widest text-blue-500">Systeem verhuist bestanden...</h2>
                <p class="text-[10px] text-zinc-500 mt-2">Niet de pagina sluiten. We verplaatsen de kluis in batches om 504 time-outs te voorkomen.</p>
            </div>
        <?php endif; ?>
        ...
        <header class="mb-16 flex justify-between items-end">
            <div>
                ...
                <p class="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mt-3">Secretaris Uil: Fase 6 - Automatisatie Hersteld</p>
            </div>
            ...
            <div class="space-y-8">
                <section class="p-8 glass rounded-[2.5rem] border border-blue-600/30 shadow-2xl shadow-blue-600/5">
                    ...
                    <form action="sync-photos.php" method="GET" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="app_name" required placeholder="bijv. Kerst 2024" 
                                   class="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-xs outline-none focus:border-blue-600 transition">
                        </div>
                ...
                <section class="p-8 glass rounded-[2.5rem]">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="sync-media.php" class="block w-full py-4 bg-zinc-900 hover:bg-zinc-800 rounded-2xl text-center text-[9px] font-black uppercase tracking-widest">🚀 Migreer naar Bucket</a>
                    </div>
                </section>
            </div>
                ...
                <section class="p-10 glass rounded-[3rem]">
                    ...
                    <div class="space-y-4">
                        <?php if (is_array($albumStats)): foreach ($albumStats as $album): ?>
                            <div class="flex items-center justify-between p-5 bg-white/5 rounded-3xl border border-white/5 group hover:border-white/10 transition">
                                ...
                                <div class="flex items-center space-x-2 shrink-0">
                                    <form action="admin.php" method="POST" class="flex space-x-2">
                                        ...
                                        <button type="submit" class="p-2 bg-zinc-800 hover:bg-blue-600 rounded-lg transition"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M5 12h14M12 5l7 7-7 7"/></svg></button>
                                    </form>
                                    <form action="admin.php" method="POST" onsubmit="return confirm('Wist ook alle bestanden in de database!')">
                                        <input type="hidden" name="action" value="delete_album"><input type="hidden" name="slug" value="<?= $album['category_name'] ?>">
                                        <button type="submit" class="p-2 bg-zinc-800 hover:bg-red-600 rounded-lg text-red-500 hover:text-white transition"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
                                    </form>
                                </div>
                            </div>
