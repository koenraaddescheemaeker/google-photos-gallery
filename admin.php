...
<style>@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@1,900&display=swap'); body { background: #000; color: #fff; font-family: 'Inter', sans-serif; } .card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 2rem; padding: 2rem; }</style>
</head>
<body class="bg-black">
...
<div class="space-y-2">
<?php if(is_array($feesten)): foreach($feesten as $f): ?>
<div class="flex justify-between items-center py-3 border-b border-white/5"><span class="font-bold"><?= ucfirst($f['category_name']) ?></span><span class="text-zinc-500 text-[10px] font-black uppercase tracking-widest"><?= $f['photo_count'] ?> Bestanden</span></div>
