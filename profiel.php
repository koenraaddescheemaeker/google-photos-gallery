<?php
/** * FORCEKES - profiel.php (Zelfbeheer) */
require_once 'config.php';
$userEmail = $_SESSION['user_email'] ?? '';
if (!$userEmail) { header("Location: admin/login.php"); exit; }

$msg = "";

// 1. Account Update (Email, Wachtwoord, Roepnaam)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_account'])) {
    $updates = [
        "nickname" => $_POST['nickname'],
        "email" => $_POST['email']
    ];
    // Manu: Wachtwoord update via Auth (indien ingevuld)
    if(!empty($_POST['password'])) { /* logica voor wachtwoord */ }
    
    supabaseRequest("members?email=eq.$userEmail", "PATCH", $updates);
    $msg = "Accountgegevens vlijmscherp opgeslagen.";
}

// 2. Album Update (Naam, Volgorde, Zichtbaarheid)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_album'])) {
    $oldSlug = $_POST['original_slug'];
    supabaseRequest("album_settings?slug=eq.$oldSlug", "PATCH", [
...
<head>
    ...
    <style>
        ...
        input, select { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #fff; padding: 0.8rem; border-radius: 1rem; width: 100%; outline: none; }
    </style>
</head>
<body class="bg-black">
    ...
    <main class="max-w-6xl mx-auto px-10 pt-48 pb-32">
        <header class="mb-12">
            ...
            <?php if($msg): ?> <p class="text-blue-500 text-[10px] font-black uppercase mt-4"><?= $msg ?></p> <?php endif; ?>
        </header>
                    ...
                    <?php foreach($myAlbums as $a): ?>
                        <form method="POST" class="bg-white/5 p-6 rounded-2xl flex flex-wrap gap-4 items-end border border-white/5">
                            ...
                            <div class="flex-1 min-w-[200px]">
                                <label class="text-[9px] text-zinc-600 block mb-2 uppercase font-black">Album Naam</label>
                                <p class="font-bold text-lg"><?= ucfirst($a['slug']) ?></p>
                            </div>
                            <div class="w-24">
                                <label class="text-[9px] text-zinc-600 block mb-2 uppercase font-black">Prio</label>
                                <input type="number" name="priority" value="<?= $a['priority'] ?>">
                            </div>
                            <div class="w-32">
                                <label class="text-[9px] text-zinc-600 block mb-2 uppercase font-black">Status</label>
