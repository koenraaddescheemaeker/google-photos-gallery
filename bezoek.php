<?php
/** * FORCEKES - bezoek.php (Fase 19: Empty State Fix - Gekeurd door Manu) */
...
<body class="bg-black">
    ...
    <main class="max-w-7xl mx-auto px-10 pt-48 pb-32">
        ...
        <?php if (empty($albums)): ?>
            <div class="py-20 border border-white/5 rounded-[3rem] bg-zinc-900/20 text-center">
                <p class="serif-italic text-2xl text-zinc-500 italic">Deze kamer is momenteel nog leeg.</p>
                <p class="text-[9px] font-black uppercase tracking-widest text-zinc-700 mt-4">Wachtend op eerste herinneringen</p>
            </div>
        <?php else: ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
                <?php foreach ($albums as $album): 
                    // Manu: vlijmscherpe check om null-fouten te voorkomen
                    $catName = $album['category_name'] ?? 'Naamloos album';
                ?>
                    <a href="gallery.php?page=<?= rawurlencode($catName) ?>" class="group relative aspect-square overflow-hidden rounded-[2rem] bg-zinc-900 border border-white/5 shadow-2xl">
                        <img src="<?= $album['thumbnail_url'] ?>" class="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-all duration-1000">
