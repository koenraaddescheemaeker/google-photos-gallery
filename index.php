<?php
/** FORCEKES - index.php (Fixed Syntax & Public) */
...
<body class="min-h-screen flex flex-col">
    ...
    <?php if ($view === 'dashboard'): ?>
        ...
        <main class="max-w-7xl mx-auto px-6 py-12 w-full mt-20">
            <header class="mb-12">
                <h1 class="text-5xl font-black italic uppercase tracking-tighter">Forcekes <span class="text-blue-600">Portaal</span></h1>
            </header>

            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <section class="glass p-8 rounded-[2.5rem]">
                    <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 mb-6">Nu in het portaal</h3>
                    <div class="space-y-4">
                        <?php if(!empty($onlineUsers) && is_array($onlineUsers)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-xs text-zinc-600 italic">Het is even rustig...</p>
                        <?php endif; ?>
                    </div>
                </section>

                <section class="lg:col-span-2 glass p-8 rounded-[2.5rem]">
                    <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 mb-8 text-blue-400">Nieuwste Herinneringen</h3>
                        ...
                        <?php if(!empty($recentPhotos) && is_array($recentPhotos)): ?>
                            <?php foreach ($recentPhotos as $img): ?>
                                <div class="group relative aspect-square rounded-3xl overflow-hidden border border-white/5 bg-zinc-900 shadow-xl">
                                    ...
                                    <div class="absolute top-3 left-3 badge px-3 py-1 rounded-full text-[8px] font-black uppercase text-white">
        ...
        <div class="flex-grow flex items-center justify-center p-6">
            <div class="w-full max-w-md glass p-12 rounded-[3.5rem] shadow-2xl">
                <div class="text-center mb-10">
                    <h1 class="text-2xl font-black italic uppercase tracking-tighter">Admin <span class="text-blue-600">Login</span></h1>
                </div>
                <?php if (isset($_GET['msg'])): ?>
                    <p class="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl text-blue-400 text-center text-xs font-bold uppercase"><?= htmlspecialchars($_GET['msg']) ?></p>
                <?php endif; ?>
